...
class W(Predicate):
    ...
    def match(self, another_word):
        """
        return True if this word's token matches
        another_word's token and this word's pos
        matches another_word's pos

        :param another_word: a Word object
        :retrun: True or False, representing if two Word matches
        """
        m1 = self.token.match(another_word.token)
        m2 = self.pos.match(another_word.pos)
        return m1 and m2
    

class Rule:
    def __init__(self, condition_num, condition=None, action=None):
        assert condition and action # both cannot be None
